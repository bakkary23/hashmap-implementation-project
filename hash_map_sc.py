...
# Assignment: 6
# Due Date: 8/9/22
# Description: This program implements a hash map data structure using separate chaining. The main
...
class HashMap:
    ...
    def put(self, key: str, value: object) -> None:
        ...
        """
        if self.get_size() == 0:  # When hash map is empty
            ...
            index = self._hash_function(key) % self.get_capacity()
            bucket = self._buckets.get_at_index(index)
            if bucket.contains(key):  # Updating existing key
    ...
    def resize_table(self, new_capacity: int) -> None:
        ...
        if new_capacity == self._capacity:  # Avoids resizing if current capacity is equivalent to new capacity
            return
        data = self.get_keys_and_values()  # Stores key/value pairs in DynamicArray object
        count = self.get_capacity()
        self.clear()
    ...
    def get(self, key: str) -> object:
        """
        Takes as a parameter a key and returns the value associated with the key
        """
        if not self.get_size():
            return None
        else:
            index = self._hash_function(key) % self.get_capacity()
            bucket = self._buckets.get_at_index(index)  # Identifies bucket with target value
    ...
    def contains_key(self, key: str) -> bool:
        ...
        if not self.get_size():
            ...
            index = self._hash_function(key) % self.get_capacity()
            bucket = self._buckets.get_at_index(index)  # Identifies bucket with target key
            if bucket.contains(key):
                return True
            else:
                return False
    ...
    def remove(self, key: str) -> None:
        ...
        if self.get_size() == 0:
            ...
            index = self._hash_function(key) % self.get_capacity()
            bucket = self._buckets.get_at_index(index)  # Identifies bucket with target key/value
            if bucket.contains(key):
                bucket.remove(key)
                self._size -= 1
            return
    ...
    def get_keys_and_values(self) -> DynamicArray:
        ...
        if not self._size:
            ...
            for index in range(self._capacity):
                if self._buckets.get_at_index(index).length():  # Checks to see if bucket is populated
                    bucket = self._buckets.get_at_index(index)
                    for node in bucket:
    ...
    def get_buckets(self):
        """
        This method returns the DynamicArray objects set to self._buckets
        """
...
def find_mode(da: DynamicArray) -> (DynamicArray, int):
    """
    This function takes as a parameter an unsorted DynamicArray object and finds the mode(s).
    It returns a tuple containing another DynamicArray object list of the mode(s) as well as the
    frequency of the mode.
    """
    map = HashMap(da.length() * 2)
    modes = DynamicArray()
    for index in range(da.length()):  # Loop creates HashMap where the key is the element and the value is its count
        key = da.get_at_index(index)
        if map.contains_key(key):  # Increment counter in HashMap frequency table
            count = map.get(key)
            count += 1
            map.put(key, count)
        else:  # Add new value to frequency table
            map.put(key, 1)
    max_freq = 0
    for index in range(map.get_capacity()):  # Adds modes to modes array
        if map.get_buckets().get_at_index(index).length():
            bucket = map.get_buckets().get_at_index(index)
            for node in bucket:
                if node.value == max_freq:
                    modes.append(node.key)
                elif node.value > max_freq:
                    max_freq = node.value
                    modes = DynamicArray()  # Resets array if mode with higher frequency is found
                    modes.append(node.key)
